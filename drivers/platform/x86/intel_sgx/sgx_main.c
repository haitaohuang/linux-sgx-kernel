...
static int sgx_drv_probe(struct platform_device *pdev)
{
	...
	if (!(fc & FEATURE_CONTROL_SGX_ENABLE)) {
		pr_err("SGX is not enabled\n");
		return -ENODEV;
	}

	cpuid(0, &eax, &ebx, &ecx, &edx);
	if (eax < SGX_CPUID) {
		pr_err("CPUID is missing the SGX leaf instruction\n");
		return -ENODEV;
	}

	cpuid_count(SGX_CPUID, SGX_CPUID_CAPABILITIES, &eax, &ebx, &ecx, &edx);
	if (!(eax & 1)) {
		pr_err("CPU does not support the SGX1 instructions\n");
		return -ENODEV;
	}
